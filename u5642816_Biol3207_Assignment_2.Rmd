...
```{r}
# Check if all the columns are the same variable class
str(meta)
```
```{r}
str(Clark_meta)
...
```{r}
lnrr <- escalc(measure="ROM", m1i = ctrl.mean, m2i = oa.mean, sd1i = ctrl.sd, sd2i = oa.sd, n1i = ctrl.n, n2i = oa.n, data=meta)
#This produces NaNs whenever the sign of ctrl.mean and oa.mean are different.
```
...
```
```{r}
temp <- lnrr %>% mutate(inverse.se = 1/sqrt(vi))
```
